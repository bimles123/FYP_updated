            ...
            <header className="flex flex-col bg-white shadow-md fixed left-0 right-0 z-50">
                <div className="flex justify-between items-center p-4">
                    ...
                    <nav className="relative flex items-center font-semibold border border-gray-300 gap-4 p-2 rounded-full bg-gray-100">
                        <div className="px-3 py-1 rounded-full hover:bg-gray-200 transition duration-200 cursor-pointer">Where in Nepal?</div>
                        <div className="border-l border-gray-300 h-6"></div>

                        <div className="relative px-3 py-1 rounded-full hover:bg-gray-200 transition duration-200 cursor-pointer" onClick={() => toggleDropdown("plan")}>
                            Plan Your Trip
                            {dropdown === "plan" && (
                                <div className="absolute top-10 left-0 w-60 bg-white shadow-lg rounded-lg p-4 z-50" onClick={handleDropdownClick}>
                                    <div className="mb-2">
                                        <label className="text-sm text-gray-600">Check-in:</label>
                                        <input type="date" value={tripDates.checkIn} onChange={(e) => handleDateChange("checkIn", e.target.value)} className="w-full border p-2 rounded mt-1" />
                                    </div>
                                    <div>
                                        <label className="text-sm text-gray-600">Check-out:</label>
                                        <input type="date" value={tripDates.checkOut} onChange={(e) => handleDateChange("checkOut", e.target.value)} className="w-full border p-2 rounded mt-1" />
                                    </div>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="border-l border-gray-300 h-6"></div>
                        <div className="relative px-3 py-1 rounded-full hover:bg-gray-200 transition duration-200 cursor-pointer" onClick={() => toggleDropdown("who")}>
                            Who's coming?
                            {dropdown === "who" && (
                                <div className="absolute top-10 left-0 w-60 bg-white shadow-lg rounded-lg p-4 z-50" onClick={handleDropdownClick}>
                                    {["adults", "children", "infants", "pets"].map((type) => (
                                        <div key={type} className="flex justify-between items-center mb-2">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <button onClick={() => updatePeopleCount(type, "decrement")} className="px-2 py-1 border rounded">-</button>
                                                <span>{people[type]}</span>
                                                <button onClick={() => updatePeopleCount(type, "increment")} className="px-2 py-1 border rounded">+</button>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                    ...
                    <button className="flex items-center gap-2 border border-gray-300 p-2 rounded-full hover:bg-gray-200 transition duration-200" onClick={toggleSidebar}>
                ...
                <div className={`fixed top-0 right-0 h-full w-64 bg-white shadow-lg transform ${isSidebarOpen ? "translate-x-0" : "translate-x-full"} transition-transform duration-300 ease-in-out`}>
                    ...
                    <div className="p-6">
                        ...
                        <button 
                            ...
                            className="px-4 py-2 text-gray-600 hover:text-black hover:bg-gray-100 transition w-full text-left"
                        >
                            Logout
                        </button>
                    </div>
